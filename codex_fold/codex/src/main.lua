...
FileLink-CoDeX Snapshot 21w01a
	...
	["greet"] = function()
		return __midblue .. "[" .. __red .. "beta" .. __white .. "-" .. __midblue .. system.version .. __darkcyan .. "@" .. __lightgreen .. __immediateDir() .. __midblue .. "]" .. __darkcyan .. " > " .. __white
	end
}
...
__file, __er = io.open("uicolor.txt", "r")
...
__lookup = {
    ...
    ["run-app"] = function()
    	...
    	max_args = 2; min_args = 2; system.checkArgs();
        os.execute('lua ' .. ' apps/' .. __current[2])
        xpcall(__main, __error)
    end,
    ["run-file"] = function()

    	max_args = 2; min_args = 2; system.checkArgs();
    	os.execute('lua ' ..  __current[2])
    	xpcall(__main, __error)
    end,
    ...
    ["reload"] = function()
        ...
        os.execute('lua /usr/local/bin/codex_fold/codex/src/main.lua')
	os.exit()
        xpcall(__main, __error)
    end,
    ["create-dir"] = function()
    ...
    ["link"] = function()
	max_args = 6; min_args = 2; system.checkArgs();
	...
	local __twolook = {
		...
		["get"] = function()
			print(__midblue .. "Retrieving Pkg Info file" .. __reset)
			os.execute("wget -O /usr/local/bin/codex_fold/codex/src/" .. __current[4] .. " https://raw.githubusercontent.com/DaarkDev/link/main/" .. __current[3])
			local file = io.open(__current[4], "r")
			if file then
				print(__yellow .. "Package found" .. __reset)
				local data = file:read("*a")
				local lines = {}
				for line in io.lines(__current[4]) do
					table.insert(lines, line)
				end
				print(__midblue .. "Fetching " .. lines[1])
				print(__midblue .. "Version " .. lines[2])
				print(os.execute("git clone --depth 1 " .. lines[3] .. " " .. __current[5] .. "/" .. lines[1]))
				local __prevdir = lfs.currentdir();
				if __current[6] == "purge" then
		...
		["ip"] = function()
			local file = io.open(__current[3], "r")
			if file then
				print(__yellow .. "Package found" .. __reset)
				local data = file:read("*a")
				local lines = {}
				for line in io.lines(__current[3]) do
					table.insert(lines, line)
				end
				print(__midblue .. "Fetching " .. lines[1])
				print(__midblue .. "Version " .. lines[2])
...
function __getApps()
	io.write(__midblue .. "Specify App Directory: " .. __reset)
	__appdir = io.read();
...
local ___conf = io.open('config.lua', 'r')
if ___conf then
	require 'config';
end
system.byteDiagnostic()
